...
test.describe('Startup', () => {
  test('opens the repo from N10_START_DIR and shows its empty state', async ({
    desktop,
  }) => {
  ...
  test('reports the app version over the bridge', async ({ desktop }) => {
    const version = await desktop.page.evaluate(() => window.n10.getVersion());
